...
const Home: FunctionComponent<HomeProps> = () => {
  ...
  const handleDeleteBook = (id: number) => {
    ...
    axios.delete(`https://0001.uz/books/${id}`, { headers : authHeadersFifth })
      ...
      .catch((error) => {
        alert.warning('Please click again')
      });
  }
                ...
                <Box className='card' width='500px' display='flex' alignItems='start'>
                  ...
                  {book.id ?
                    <div className='actions' style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginTop: '40px' }}>
                      <div key={book?.id} onClick={() => { handleDeleteBook(book?.id); }} style={{ width: '32px', height: '32px', background: '#FF4D4F', borderRadius: '6px 6px 6px 0', cursor: 'pointer', padding: '6px 5px' }}>
