...
const Home: FunctionComponent<HomeProps> = () => {
  ...
  const { authHeadersFirst, authHeadersThird, authHeadersFifth, authHeadersSixth } = useAuthHeaders({ isbn, deleteId, editId, status });
  ...
  return (
    <>
      ...
      <Box padding='0 100px' mt='36px'>
        ...
        <Grid mt='30px' className='grid-scroll' container spacing={2}>
          {bookData?.map((book, index) => {
            return (
              <Grid item key={index} xs={4}>
                <Box className='card' width='500px' display='flex' alignItems='start'>
                  <Card sx={{ marginTop: '20px', transition: 'all 0.2s', width: '397px', height: '220px', padding: '32px', display: 'flex', flexDirection: 'column', gap: '16px', borderRadius: '12px', }} variant="outlined">
                    ...
                    <Box display='flex' alignItems='center' justifyContent='space-between'>
                      ...
                      <Badge sx={{ width: '110px', padding: '4px 10px', height: '30px', borderRadius: '8.5px', bgcolor: '#EFE6FD', color: '#9654F4' }}>Pages : {book.pages}</Badge>
                    </Box>
                  </Card>

                  {book.id ?
                    <div className='actions' style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginTop: '40px' }}>
                      <div key={book?.id} onClick={() => { setDeleteId(book?.id); handleDeleteBook(book?.id); }} style={{ width: '32px', height: '32px', background: '#FF4D4F', borderRadius: '6px 6px 6px 0', cursor: 'pointer', padding: '6px 5px' }}>
                ...
                <Modal open={openEdit} onClose={handleCloseEdit} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
                  <Box sx={modalStyle}>
                    ...
                    <Box display='flex' gap='20px' marginTop='20px'>
                      ...
                      <Button onClick={() => { handleOpenEdit(); handleEditBook(book.id); setEditId(book.id) }} disabled={loading} sx={{ width: '200px', bgcolor: '#6200EE', fontSize: '16px', fontFamily: 'Mulish', color: '#FEFEFE', textTransform: 'none', "&:hover": { background: '#6200EE' } }}>{loading ? 'Loading...' : 'Submit'}</Button></Box>
                  </Box>
                </Modal>
              </Grid>
            )
